-- =============================================================================
-- PeoplePulse — Database Schema Migration 026
-- Zero-Friction Employee Onboarding & Instant Account Auto-Provisioning
-- =============================================================================

-- 1. Ensure extensions pgcrypto is accessible
create extension if not exists pgcrypto with schema extensions;

-- 2. Grant unauthenticated preview and acceptance privileges
grant execute on function public.get_invitation_details(text) to anon, authenticated;

-- 3. Core RPC: provision_and_accept_invitation
-- Allows invited employees to set their password, auto-create/verify their auth account,
-- and accept the invitation in ONE ATOMIC STEP without Supabase signup email rate limits!
...
begin
  ...
  if v_existing_user_id is not null then
    -- User already exists: update password directly & ensure confirmed
    ...
    insert into auth.identities (
      ...
      v_user_id,
      v_user_id,
      json_build_object('sub', v_user_id, 'email', lower(v_invite.email))::jsonb,
      'email',
      v_user_id,
      now(),
      now(),
...
-- Also ensure accept_org_invitation is granted to anon and authenticated
