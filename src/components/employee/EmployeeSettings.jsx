...
export default function EmployeeSettings({ setMobileOpen, onSignOut, onReturnHome }) {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      else if (onReturnHome) onReturnHome();
  ...
  return (
    <div>
      <Topbar title="Settings" subtitle="Manage your account preferences and privacy." setMobileOpen={setMobileOpen} />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          ...
          <div className="space-y-3">
            ...
            {profile?.employee_id && (
              <div>
                <label className="text-xs font-medium block mb-1" style={{ color: T.muted }}>Employee ID</label>
                <input
                  readOnly
            ...
            <div>
              <label className="text-xs font-medium block mb-1" style={{ color: T.muted }}>Organization</label>
              <input
                readOnly
        ...
        <Card>
          <p className="text-base font-semibold mb-3" style={{ color: T.text }}>Notification Preferences</p>
          <div className="flex items-center gap-2 mb-3 text-xs" style={{ color: T.muted }}>
            <Bell size={14} style={{ color: T.accent }} />
            Control how and when you receive notifications.
          </div>
          <ToggleRow label="Daily check-in reminders" sub="Get a push notification every morning" defaultOn />
          <ToggleRow label="Email notifications" sub="Receive weekly wellbeing summary via email" defaultOn />
          <ToggleRow label="Engagement alerts" sub="Get notified about team events and updates" defaultOn />
        </Card>

        {/* Privacy Settings */}
        <Card>
          <p className="text-base font-semibold mb-3" style={{ color: T.text }}>Privacy Settings</p>
          <div className="flex items-center gap-2 mb-3 text-xs" style={{ color: T.muted }}>
            <ShieldCheck size={14} style={{ color: T.positive }} />
            Your responses are always encrypted and secure.
          </div>
          <ToggleRow label="Submit check-ins anonymously" sub="Hide your identity from managers on pulse responses" defaultOn />
          <ToggleRow label="Show profile to team members" sub="Let colleagues see your name and role in the team directory" defaultOn />
        </Card>

        {/* Appearance */}
        <Card>
          <p className="text-base font-semibold mb-3" style={{ color: T.text }}>Appearance</p>
          <ToggleRow label="Compact view" sub="Reduce spacing for denser information display" />
          <ToggleRow label="Show motivational quotes" sub="Display daily inspirational messages on your dashboard" defaultOn />
        </Card>
